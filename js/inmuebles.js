...
const PARALLAX_IMGS = {
  ...
  constructoraBg: 'https://images.unsplash.com/photo-1590725140246-20acdee442be?auto=format&fit=crop&w=1920&q=80', // ✅ NUEVA
...
window.verDetalle = async (id) => {
  ...
  const miniaturas = fotos.length > 1
    ? fotos.map((f, idx) => `
        <img src="${f}"
             ...
             loading="lazy"
             alt="foto ${idx + 1}"
             onclick="cambiarImg(this, '${f}')">`).join('')
    : '';

  // ✅ Botones de navegación
  const navButtons = fotos.length > 1 ? `
    <button type="button" class="thumb-nav prev" onclick="scrollThumbs(-1)">
      <i class="fas fa-chevron-left"></i>
    </button>
    <button type="button" class="thumb-nav next" onclick="scrollThumbs(1)">
  ...
  const infoRowsHTML = `
    <div class="extra-info">
      <div class="info-row"><span>Tipo de inmueble</span><strong>${item.tipo_inmueble || 'No especificado'}</strong></div>
      ${item.paga_administracion ? `<div class="info-row"><span><i class="fas fa-file-invoice-dollar" style="color:var(--gold);margin-right:5px"></i>Administración</span><strong>${item.valor_administracion ? '$' + Number(item.valor_administracion).toLocaleString('es-CO') : 'Incluida'}</strong></div>` : ''}
    </div>
  `;
  ...
  // ✅ HTML COMPLETO - CON BOTÓN CERRAR CORRECTO
  container.innerHTML = `
    <div class="dialog-container">
      ...
      <div class="dialog-grid">
        <div class="dialog-gallery">
          ...
          ${miniaturas ? `
            <div class="thumb-wrapper">
              ${navButtons}
              <div class="thumb-list" id="thumbList">${miniaturas}</div>
            </div>
          ` : ''}
        </div>
        ...
        <div class="dialog-info">
          ...
          ${infoRowsHTML}
          ${extrasHTML}
          ${item.descripcion ? `<div class="desc-box"><h3>Descripción</h3><p>${escapeHtml(item.descripcion)}</p></div>` : ''}
          <a href="https://wa.me/573156376306?text=Hola,%20solicito%20información%20sobre:%20${encodeURIComponent(item.titulo)}%20en%20${encodeURIComponent(ubicacion)}" target="_blank" class="btn-wpp-dialog"><i class="fab fa-whatsapp"></i> Preguntar por WhatsApp</a>
        </div>
      </div>
    </div>`;
...
window.cambiarImg = function(thumb, src) {
...
// ✅ Función para scroll de thumbnails (FLECHAS)
window.scrollThumbs = function(direction) {
  const thumbList = document.getElementById('thumbList');
  if (!thumbList) {
    console.log('No se encontró thumbList');
    return;
  }
  
  console.log('Scroll direction:', direction); // Debug
  thumbList.scrollBy({
    left: direction * 140,
    behavior: 'smooth'
  });
